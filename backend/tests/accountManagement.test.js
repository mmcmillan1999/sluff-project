...
async function testDeleteHappyPath() {
    ...
    // Personal data keyed to the id or the name goes too: reaction-time
    // telemetry, the bidder link on round analytics, and the spoken-name clip
    // for every name the player has used.
    assert.ok(sql.some(text => text.startsWith('DELETE FROM play_timings')));
