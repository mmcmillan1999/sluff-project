...
async function testDrawRequestLifecycle() {
    ...
    assert.strictEqual(engine.drawRequest.isActive, true);
    ...
    await effectProcessor.processEffects(engine, finalVoteResult.effects);
    ...
    await effectProcessor.processEffects(engine, mixedVoteResult.effects);
    assert.strictEqual(engine.state, "Game Over");
...
async function runAllTests() {
    try {
        ...
        await testGameOverPayouts(); // Re-enabled
        await testWidowAssignmentLogic();
    } catch (error) {
