...
class MarketInsuranceStrategy {
    ...
    calculateInsuranceMove(engine, bot) {
        ...
        const { samples } = estimateBidderPoints(view, { rollouts: this.rollouts });
        ...
        if (isBidder) {
            ...
            ask = clamp(ask, -120 * m, 120 * m);
            const current = insurance.bidderRequirement ?? 0;
            if (Math.abs(ask - current) >= MIN_EMIT_DELTA) {
                return { settingType: 'bidderRequirement', value: ask };
            }
            return null;
        }
