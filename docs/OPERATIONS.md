...
| Email (transactional) | **Resend** (only provider; the SendGrid rail was removed Sept 2026) | _fill in account email_ | Sends as noreply@playsluff.com. `RESEND_API_KEY` env var (preferred). **June 2026: switched to Resend after SendGrid ran out of credits. `playsluff.com` re-verified in Resend (needs SPF/DKIM DNS records live) — registration verification + password-reset emails working again.** If sends start 403'ing, re-check domain verification at resend.com/domains. |
