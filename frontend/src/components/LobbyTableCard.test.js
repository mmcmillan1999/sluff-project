...
test('retains theme identity without rendering venue copy in a private table card', () => {
    ...
    expect(screen.getByText('Untied returns: 3P 2×/1×/0× · 4P 2.5×/1×/0.5×/0×')).toBeInTheDocument();
    ...
    expect(container.querySelector('.table-economics')).toHaveAttribute(
        'aria-label',
        'Buy-in 20 tokens. Untied returns are 2, 1, and 0 times the buy-in in a three-player game, or 2.5, 1, 0.5, and 0 times in a four-player game. Ties are settled from the same pot.'
    );
