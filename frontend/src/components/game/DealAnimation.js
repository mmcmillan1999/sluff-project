...
const flightGeometry = (source, target, index) => {
    ...
    const arc = Math.min(42, Math.max(14, distance * 0.08)) * arcDirection;
    const perpendicularX = -dy / distance;
    const perpendicularY = dx / distance;
    const rotationDirection = index % 2 === 0 ? 1 : -1;
    const startRotation = ((index * 7) % 13) - 6;

    return {
        '--deal-flight-duration': `${DEAL_CARD_FLIGHT_MS}ms`,
        '--deal-start-x': `${source.x - (cardWidth / 2)}px`,
        '--deal-start-y': `${source.y - (cardHeight / 2)}px`,
        '--deal-mid-x': `${((source.x + target.x) / 2) + (perpendicularX * arc) - (cardWidth / 2)}px`,
        '--deal-mid-y': `${((source.y + target.y) / 2) + (perpendicularY * arc) - (cardHeight / 2)}px`,
        '--deal-end-x': `${target.x - (cardWidth / 2)}px`,
        '--deal-end-y': `${target.y - (cardHeight / 2)}px`,
        '--deal-start-rotation': `${startRotation}deg`,
        '--deal-mid-rotation': `${startRotation + (26 * rotationDirection)}deg`,
        '--deal-end-rotation': `${startRotation + (48 * rotationDirection)}deg`,
    };
};
        ...
        const launch = (step, index) => {
            ...
            setFlights((current) => [
                ...current,
                {
                    ...
                    style: flightGeometry(sourcePoint, targetPoint, index),
                },
            ]);
